...
If you discover a security vulnerability in xcbot, please report it by:
...
chmod 600 ~/.xcbot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.xcbot/config.json` with file permissions set to `0600`
...
- ✅ Run xcbot with a dedicated user account
...
- Keep authentication data in `~/.xcbot/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for xcbot and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash xcbot
   sudo -u xcbot xcbot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.xcbot
   chmod 600 ~/.xcbot/config.json
   chmod 700 ~/.xcbot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.xcbot/logs/xcbot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.xcbot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.xcbot/logs/xcbot.log
   ```
